...
Vedremo un esempio con una _struct_ di un post del blog che ha un campo per
...
Cominciamo implementando passo passo un flusso di lavoro per un post del blog
usando lo _state pattern_.

La funzionalità finale sarà questa:

1. Un _post_ inizia come bozza vuota.
1. Quando la bozza è pronta, si richiede la revisione del post.
1. Quando il post è approvato, viene pubblicato.
1. Solo i post pubblicati restituiscono contenuto da stampare, quindi i post non
   approvati non possono essere pubblicati accidentalmente.

Qualsiasi altra modifica tentata su un post non avrà effetto. Per esempio, se
proviamo ad approvare una bozza prima di richiedere la revisione, il post
resterà una bozza non pubblicata.

### Un Tentativo Tradizionale Orientato agli Oggetti
...
privato `Stato` che definisce il comportamento che tutti gli oggetti stato
devono avere.

`Post` terrà un oggetto _trait_ `Box<dyn Stato>` dentro un `Option<T>` in un
campo privato chiamato `stato`. Vediamo dopo perché serve `Option<T>`.
...
Il _trait_ `Stato` definisce il comportamento condiviso tra gli stati del post.
Gli oggetti stato sono `Bozza`, `AttesaRevisione` e `Pubblicato`, e implementano
tutti `Stato`. Per ora il _trait_ è vuoto; inizieremo con `Bozza` perché è lo
...
Vediamo da Listato 18-11 che vogliamo avere un metodo `aggiungi_testo` che
prende un `&str` e lo aggiunge al contenuto del post. Lo facciamo come metodo,
non esponendo il campo `contenuto` come `pub`, così poi possiamo controllare
come leggerlo. Il metodo `aggiungi_testo` è semplice; lo aggiungiamo nel blocco
`impl Post` come nel Listato 18-13.

<Listing number="18-13" file-name="src/lib.rs" caption="Implementazione del metodo `aggiungi_testo` per aggiungere testo al `contenuto` del post">
...
Il metodo `aggiungi_testo` prende un riferimento mutabile a `self` perché
modifica l’istanza `Post`. Chiama `push_str` sulla stringa `contenuto`,
aggiungendo il testo. Questo comportamento non dipende dallo stato, quindi non
fa parte dello _state pattern_. `aggiungi_testo` non tocca il campo `stato`.
...
post, vogliamo che il metodo `contenuto` ritorni una _slice_ vuota perché il
post è ancora una bozza (linea 7 di Listato 18-11). Per ora implementiamo
`contenuto` che semplicemente restituisca una _slice_ vuota. Lo cambieremo in
seguito quando aggiungeremo la possibilità di cambiare stato per la
pubblicazione. Per ora, i post possono essere solo bozze, e quindi `contenuto`
ritorna sempre una stringa vuota. Il Listato 18-14 mostra questa semplice
definizione.

<Listing number="18-14" file-name="src/lib.rs" caption="Implementazione segnaposto del metodo `contenuto` di `Post` che restituisce sempre una _slice_ vuota">
...
Ora dobbiamo aggiungere la funzionalità per richiedere una revisione di un post,
che dovrebbe cambiare il suo stato da `Bozza` a `AttesaRevisione`. Il Listato
18-15 mostra questo codice.
...
riferimento mutabile a `self`. Poi chiamiamo un metodo interno
...
di avere `self`, `&self` o `&mut self` come primo parametro, abbiamo `self:
Box<Self>`. Questa sintassi significa che il metodo è valido solo chiamandolo su
una `Box` che contiene il _type_. Questa sintassi prende _ownership_ di
`Box<Self>`, invalidando il vecchio stato in modo che il valore di stato del
...
da `Post` invece di prenderlo in prestito. Poi assegniamo al campo `stato` del
post il risultato di questa operazione.
...
stato di un post in attesa di revisione. Anche la _struct_ `AttesaRevisione`
...
Qui si vedono i vantaggi dello _state pattern_: il metodo `richiedi_revisione`
...
restituisce se stessa perché il post dovrebbe rimanere nello stato `Pubblicato`
in quei casi.
...
- Aggiungi un metodo `respingi` che cambia lo stato del post da
...
tentativi di usare post in bozze dove sono permessi solo post pubblicati,
...
post bozza in produzione perché quel codice nemmeno si compila. Il Listato 18-19
mostra la definizione di una _struct_ `Post` e una `PostBozza`, oltre ai metodi
su ciascuna.
...
memorizza il testo del post. Le _struct_ non hanno più il campo `stato` perché
