...
aggiunto una nuova riga con il testo `"Duttilità."` usando una _D_ maiuscola che
...
esistenti. Supponiamo che la query sia `"rUsT"`, ad esempio: quella _slice_ non
...
di `contains` è definita per accettare una _slice_ di stringa.
...
_case-sensitive_ a quella _case-insensitive_. L’aggiunta di questo campo causerà
...
che è già nello _scope_ all’inizio di _src/main.rs_. Useremo la funzione `var`
...
interessa il _valore_ della variabile d’ambiente, ma solo se è impostata o meno,
...
Dovremmo ottenere righe che contengono _che_ e che potrebbero contenere lettere
maiuscole:
...
programma `minigrep` ora può effettuare ricerche _case-insensitive_, controllate
...
la precedenza. Come esercizio e per sperimentare un po', prova a controllare la
distinzione tra maiuscole e minuscole tramite un argomento della riga di comando
o una variabile d’ambiente. Decidi se l’argomento della riga di comando o la
variabile d’ambiente debbano avere la precedenza se il programma viene eseguito
con uno impostato _case-sensitive_ e l’altro impostato come _case-insensitive_.

Il modulo `std::env` contiene molte altre utili funzionalità per gestire le
variabili d’ambiente: consulta la sua [documentazione][std] per scoprire quali
sono disponibili.
